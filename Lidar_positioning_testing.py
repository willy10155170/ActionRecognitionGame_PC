...
def W_cor_Erfix(World_cor):
    for i in range(33):
        if(World_cor[i][3] == 0.0):
            World_cor[i] = History_joint[i]
        else:
            History_joint[i] = World_cor[i]
    return World_cor,History_joint
...
def Cal(image,lmList,aligned_depth_frame,color_intrin,ht,Direc):
    ...
    try:
        if len(lmList) !=0:
            ...
            World_cor, History_joint = W_cor_Erfix(World_cor)
